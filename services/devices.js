...
module.exports = {
  ...
  searchByText: async (tid, sText, limit, access_token) => {
    const data = await instance.get(`/tenant/devices/${tid}?limit=${limit}&textSearch=${sText}`, {
      headers: {
  ...
  getDeviceData: async (id, access_token) => {
    ...
    const data = await instance.get(`/data/alllatestdata/${deviceId}`, {
      headers: {
        Authorization: `Bearer ${ access_token}`,
      }
    });
    const yaoce = data.data;
  ...
  siteDevicesSearch: async (tid, siteId, limit, textSearch, access_token) => { // 待定,底层接口有问题
    try {
      const data = await instance.get(`/sitedevices/${tid}/${siteId}?limit=${limit}&textSearch=${textSearch}`, {
        headers: {
          Authorization: `Bearer ${access_token}`,
        }
      });
  ...
  assignDevicetoSite: async (id, siteId, access_token) => {
    try {
      let res = await request.put('http://deviceaccess:8100/api/v1/deviceaccess/device')
